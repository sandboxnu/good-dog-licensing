...
export default function AdminLanding() {
  ...
  return <div className="flex flex-row gap-[24px] w-[1360px]">
...
<div className={`rounded-[4px] flex items-center justify-center h-[16px] w-[23px] ${active ? "bg-grass-green-50" : "bg-gray-500"}`}>
  <p className={`${active ? "text-dark-gray-500" : "text-gray-100"} text-[14px] font-medium leading-none`}>{number}</p>
</div>     
 </div>
    <p className={`text-caption leading-[96%] ${active ? "text-gray-100" : "text-dark-gray-500"}`}>{subtitle}</p>
  </div>
}
...
{data.map((project : ProjectSubmissionWithSongRequestAndMatches) => {
  return (<div className="p-[16px] bg-white flex flex-row gap-[50px] border-[0.2px] border-t-0 border-solid border-cream-400">
    <p className="flex-1">{project.projectTitle}</p>
    <p className="flex-1">{project.description}</p>
    <p className="flex-1">{project.projectOwnerId}</p>
    <p className="flex-1">{project.createdAt.toISOString()}</p>
...
function SideBar({
  ...
  return <div className="flex bg-white flex-1 w-[207px] h-[776px] pt-[32px] pl-[16px] pr-[16px] shadow-[0_2px_6px_0_#ECE6DF] rounded-[24px] h-[40px]">
    <div className="flex flex-col gap-[4px]">
      <SideBarEntry active={activeTab === "submissions"} text={"Submissions"} icon={<SubmissionTabIcon active={activeTab === "submissions"}/>} onClick={() => setActiveTab("submissions")}/>
      <SideBarEntry active={activeTab === "songs"} text={"Songs"} icon={<SongTabIcon active={activeTab === "songs"}/>} onClick={() => setActiveTab("songs")}/>
      <SideBarEntry active={activeTab === "users"} text={"Users"} icon={<UsersTabIcon active={activeTab === "users"}/>} onClick={() => setActiveTab("users")}/>
    </div>
  </div>
}
...
function SideBarEntry({
  ...
  return <div className={`${active ? "bg-[#07634C] text-[#FFFFFF]" : "bg-white text-black"} items-center pl-[8px] pt-[8px] pb-[8px] pr-[32px] w-[175px] rounded-[8px]`} onClick={onClick}>
    <div className="flex flex-row gap-[8px]">
      {icon}
      <p className={`text-body2 ${active ? "text-white" : "text-black"} leading-[128%]`}>{text}</p>
    </div>
  </div>
}